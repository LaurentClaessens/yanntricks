...
class SudokuGridGraph(ObjectGraph):
    ...
    def action_on_pspict(self, pspict):
        ...
        for i in range(0, 9):
            ...
            A.parameters.symbol = ""
            A.put_mark(0, 0, string.ascii_uppercase[i], pspict=pspict)
        ...
        # Grid
        for i in range(0, 10):
            v = Segment(Point(i*self.length, 0),
                        Point(i*self.length, -9*self.length))
            h = Segment(Point(0, -i*self.length),
                        Point(9*self.length, -i*self.length))
            # for the subgrid
            if i % 3 == 0:
        ...
        for i, li in enumerate(lines):
            for j, c in enumerate(li.split(",")):
                A = Point(j*self.length+self.length /
                          2, -i*self.length-self.length/2)
                A.parameters.symbol = ""
                if c == "i":
                    A.put_mark(3*self.length/9, text="\ldots",
                               pspict=pspict, position="N")
