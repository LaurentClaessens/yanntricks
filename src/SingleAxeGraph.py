...
class SingleAxe(ObjectGraph):
    ...
    - ``self.Dx`` - (default=1)
        A mark is written each multiple of ``self.Dx*base``.
    - ``self.mark_angle``
        the angle in degree under which the mark are written.
        By default this is orthogonal to the direction given by
        `self.base`.

        If an user-defined axes_unit is given, the length of
        ``base`` is "forgotten"
        """
    ...
    def conclude(self, pspict):
        ...
        the length of the axes are unknown up to the last moment.
        The axe is created in the same time as the picture,
        but the mark bars for example can only be computed after
        all every other objects.So we need to make some ultimate
        settings before to be drawn.
        This function is called by `Picture._DrawGraph`, right before
        to compute and add the bounding box and to call
        `action_on_pspict`.
        """
        if self.mark:
    ...
    def action_on_pspict(self, pspict):
        ...
        v = self.segment(pspict=pspict).affine_vector()
