...
import phystricks.src.BasicGeometricObjects
...
class FigureGenerationSuite(object):
    ...
    def latex_portion(self, failed_list, lstinputlisting=False):
        ...
        for a in failed_list:
            try:
                ...
                try:
                    # In the case we are arriving here to create the documentation.
    ...
    def summary(self):
        ...
        if len(self.to_be_recompiled_list) != 0:
            ...
            print(self.function_list_to_figures_list(
                self.to_be_recompiled_list))
            self.create_to_be_latex_file(
                self.to_be_recompiled_list, name="recompiled")
            all_tests_passed = False
        if all_tests_passed:
