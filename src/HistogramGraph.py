...
class HistogramGraph(ObjectGraph):
    def __init__(self, tuple_box_list, legende=None):
        ...
        self.d_xmin = min([b.d_xmin for b in self.box_list]
    ...
    def action_on_pspict(self, pspict):
        ...
        for xx in values:
            ...
            P.parameters.symbol = "|"
            # see 71011299 before to change this 0.2
            P.put_mark(0.2, text=str(xx), pspict=pspict, position="N")
            pspict.DrawGraphs(P)
        for box in self.box_list:
            P = box.rectangle.segment_N.mark_point()
            P.put_mark(0.2, text="$"+str(box.n)+"$",
                       pspict=pspict, position="S")
            P.parameters.symbol = ""
            pspict.DrawGraphs(P)
