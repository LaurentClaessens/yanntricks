...
# copyright (c) Laurent Claessens, 2010,2012, 2019
# email: laurent@claessens-donadello.eu

"""
This module uses yanntricks in order to produces some mathematical situations.
"""
...
class Dynkin(object):
    ...
    def action_on_pspict(self, pspict):
        ...
        for i in range(0, n):
            self.MarkThePoint(
                self.description.ronds[i],
                self.decoration.distMark[i],
                self.decoration.angleMark[i],
                self.description.remplissure[i],
                self.decoration.visMark[i])
...
class Description_Dynkin(object):
    def __init__(self, ll):
        ...
        for i in range(0, lAdj):
            z = []
            for j in range(0, lAdj):
                z.append(0)
            self.Adjacence.append(z)
        for i in range(0, int(float(n)/2)):
            if ll[2*i+1] == "-":
                self.Adjacence[i][i+1] = 1
            if ll[2*i+1] == "=":
                self.Adjacence[i][i+1] = 2
            if ll[2*i+1] == ".":
                self.Adjacence[i][i+1] = 4
...
class NewtonMethodStep():
    ...
    def __init__(self, newton, xn):
        ...
        # The Newton's iteration formula is here
        xnn = xn - (self.P.y)/newton.f.derivative()(xn)
        self.B = Point(xnn, 0)
    ...
    def TraceYoung(self, Y):
        for i in range(0, len(Y.diagramme)):
            for j in range(0, len(Y.diagramme[i])):
                self.TraceRectangle(
                    Rectangle(Point(j, -i), Point(j+1, -i-1)), "")
                self.MarkThePoint(Point(j, -i), 0.5, -45,
                                  "", Y.diagramme[i][j])
