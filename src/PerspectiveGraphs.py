...
from phystricks.src.Defaults import CIRCLE3D_LINEAR_PLOTPOINTS
...
class CuboidGraph(ObjectGraph):
    def __init__(self, op, P, a, b, c):
        ...
        self.A = [Point(self.Px, self.Py+b), Point(self.Px+a, self.Py+b),
                  Point(self.Px+a, self.Py), Point(self.Px, self.Py)]
        ...
        # The edges.
        self.segP = [Segment(self.c1[i], self.c2[i])
                     for i in range(0, len(self.c1))]
        self.segc1 = [Segment(self.c1[i], self.c1[(i+1) % len(self.c1)])
                      for i in range(0, len(self.c1))]
        self.segc2 = [Segment(self.c2[i], self.c2[(i+1) % len(self.c2)])
    ...
    def action_on_pspict(self, pspict):
        ...
        if not self.transparent:
            ...
            if self.op.alpha < 90:
                surface3 = Polygon(
                    self.c1[1], self.c2[1], self.c2[2], self.c1[2])
            else:
                surface3 = Polygon(
                    self.c1[0], self.c2[0], self.c2[3], self.c1[3])
            surface3.filled()
