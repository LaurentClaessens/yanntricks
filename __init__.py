...
class figure(object):
	...
	def conclude(self):
		...
		for f in self.SSfigures :
			...
			self.AjouteLigne("}					% Fermeture de la sous-figure "+str(self.SSfigures.index(f)+1))
			self.AjouteLigne("%")
...
def AjouteLigne_entete(truc):
	truc.AjouteLigne("% This file is automatically generated by phystricks")
	truc.AjouteLigne("%  see the documentation ")
	truc.AjouteLigne("%  http://student.ulb.ac.be/~lclaesse/phystricks-doc.pdf ")
	truc.AjouteLigne("%  See the projects phystricks, phystricks-doc and LaTexparser at ")
	truc.AjouteLigne("%  http://gitorious.org/~moky")
...
class pspicture(object):
	...
	class _DrawPoint(object):
		def __init__(self,picture,P,symbol,params):
			...
			self.picture.AjouteLigne( P.code(params_mettre) )
		def MarkThePoint(self,dist,angle,marque):
			if self.P.psNom not in self.picture.listePoint :
				self.picture.AddPoint(self.P)
			self.picture.AjouteLigne("\\rput("+self.P.psNom+"){\\rput("+str(dist)+";"+str(angle)+"){"+marque+"}}")
	...
	def __init__(self,name="CAN_BE_A_PROBLEM_IF_TRY_TO_PRODUCE_EPS"):
		...
		self.AjouteLigne("%GRID")	# A \n is automatically added.
		self.AjouteLigne("%AXES")
		self.AjouteLigne("%OTHER STUFF")
	...
	def AddPoint(self,P):
		self.AjouteLigne(self.CodeAddPoint(P))
	...
	def TraceNuage_de_Points(self,nuage,symbol,params):
		self.AjouteLigne("% ---------Nuage de point--------")
		for P in nuage.listePoints :
			self.DrawPoint(P,symbol,params)

	def MarqueAngle(self,A,B,C,label,params):
		self.AjouteLigne("\pstMarkAngle["+params+"]{"+A.psNom+"}{"+B.psNom+"}{"+C.psNom+"}{"+label+"}")
	...
	def TraceSurfacephyFunction(self,surf):
		...
		self.BB.AddPoint(B)
		self.BB.AddphyFunction(f,mx,Mx)
		self.AjouteLigne("\pscustom["+opt.code()+"]{")
		self.AjouteLigne("\psline"+A.coordinates()+X.coordinates())
		self.AjouteLigne("\psplot{"+str(mx)+"}{"+str(Mx)+"}{"+f.pstricks+"}")
		self.AjouteLigne("\psline"+Y.coordinates()+B.coordinates())
		self.AjouteLigne("}")
	def TraceSurfaceEntrephyFunction(self,surf):
		...
		self.AjouteLigne("\pscustom["+opt.code()+"]{")
		self.AjouteLigne("\psplot[plotstyle=curve]{"+str(mx)+"}{"+str(Mx)+"}{"+f.pstricks+"}")
		self.AjouteLigne("\psplot[liftpen=1]{"+str(Mx)+"}{"+str(mx)+"}{"+g.pstricks+"}")
		self.AjouteLigne("}")
	...
	def TracephyFunction(self,fun,min,max,params):
		...
		self.AjouteLigne("\psplot["+params+"]{"+str(deb)+"}{"+str(fin)+"}{"+fun.pstricks+"}")

	def DrawGraphOfAPoint(self,graphe):
		p = graphe.point
		self.BB.AddPoint(p)
		self.AjouteLigne( p.code(graphe.params()) )
		if graphe.marque :
			mark = graphe.mark
			if p.psNom not in self.listePoint :
				self.AddPoint(p)
			#self.AjouteLigne("\\rput("+p.psNom+"){\\rput("+str(dist)+";"+str(angle)+"){"+marque+"}}")
			self.AjouteLigne("\\rput(%s){\\rput(%s;%s){%s}}"%(p.psNom,str(mark.dist),str(mark.angle),str(mark.mark)))
	...
	def TraceTriangle(self,tri,params):
		...
		self.AjouteLigne("\pstTriangle["+params+",PointSymbol=none]"+tri.A.coordinates()+"{A}"+tri.B.coordinates()+"{B}"+tri.C.coordinates()+"{C}")
		
	def TraceRectangle(self,rect,params):
		self.BB.AddPoint(rect.bg)
		self.BB.AddPoint(rect.hd)
		self.AjouteLigne("\psframe["+params+"]"+rect.hd.coordinates()+rect.bg.coordinates())
	def TraceCircle(self,Cer,params):
		...
		self.AddPoint(PsA)
		self.AjouteLigne("\pstCircleOA["+params+"]{"+Cer.centre.psNom+"}{"+PsA.psNom+"}")
		# La commande pscircle ne tient pas compte des xunit et yunit => inutilisable.
		#self.AjouteLigne("\pscircle["+params+"]("+Cer.centre.psNom+"){"+str(Cer.rayon)+"}")
	def TraceArcCircle(self,Cer,angleI,angleF,params):
		...
		self.AddPoint(PsA)
		self.AddPoint(PsB)
		self.AjouteLigne("\pstArcOAB[%s]{%s}{%s}{%s}"%(params,Cer.centre.psNom,PsA.psNom,PsB.psNom))
	...
	# 	pour que la BB en tienne compte. C'est pour ça que DrawAxes fait appel à IncrusteLigne et non AjouteLigne.
	#
